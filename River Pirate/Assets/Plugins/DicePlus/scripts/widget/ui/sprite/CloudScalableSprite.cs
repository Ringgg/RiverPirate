using UnityEngine;
//five element x dimension scalable sprite
//elements are left edge, space between left edge and arrow, arrow, space between arrow and right edge, right edge
public class CloudScalableSprite : SimpleSprite
{
	//texture coordinates for elements
	[HideInInspector]
	public float [] uvElements = new float[6] {
		0f, 0.0625f, 0.1458f, 0.271f, 0.9375f, 1f
	};
	//width of elements, positive values are "exact" pixel values, negative values are ratios at which remaining width is divided
	[HideInInspector]
	public float [] elements = new float[5] {
		6, -1, 12, -1, 6
	};
	...
	public float getVariablePartLength() {
		float sum = 0;
		float ratioSum = 0;
		for (int i = 0; i < uvElements.Length - 1; i++) {
			if (elements[i] > 0) {
		...
		return Mathf.Max(0, trans.localScale.x - sum);
	}
}